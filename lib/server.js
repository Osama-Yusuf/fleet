...
function checkHiveCompliance(hivePath, bees) {
  const issues = [];
  const hasQueen = fs.existsSync(path.join(hivePath, '.fleet', 'queen'));
  if (!hasQueen) issues.push({ type: 'queen_dir', message: 'Missing .fleet/queen/ directory' });

  let hasMcpProtocol = false;
  try {
    const brain = fs.readFileSync(path.join(hivePath, 'CLAUDE.md'), 'utf8');
    hasMcpProtocol = brain.includes('fleet_ping') || brain.includes('fleet_claim') || brain.includes('Session Coordination') || brain.includes('Coordination (Queen-managed)');
  } catch {}
  if (!hasMcpProtocol) issues.push({ type: 'protocol', message: 'CLAUDE.md missing MCP coordination protocol' });

  const beesWithoutMcp = [];
  for (const bee of bees) {
    if (!fs.existsSync(path.join(bee.path, '.mcp.json'))) beesWithoutMcp.push(bee.name);
  }
  if (beesWithoutMcp.length) issues.push({ type: 'mcp_config', message: `Missing .mcp.json: ${beesWithoutMcp.join(', ')}`, bees: beesWithoutMcp });
...
function hiveData(hivePath) {
  ...
  const compliance = checkHiveCompliance(hivePath, bees);
  return { ...fleet, bees, path: hivePath, compliance };
}
...
function runFleet(hivePath, args) {
  ...
  try {
    const raw = execSync(`cd "${hivePath}" && "${fleetBin}" ${args}`, {
      encoding: 'utf8', timeout: 30000, input: 'y\n',
    });
    return stripAnsi(raw);
  } catch (e) {
    return stripAnsi(e.stdout || e.message);
  }
}
...
const MCP_TOOLS = [
  { name: 'fleet_ping',
    description: 'Heartbeat — MUST call on session start and every 60 seconds after. Returns Queen status, pending instructions, escalations, and what other bees are currently working on (activeBees). The Queen detects stale bees by missing heartbeats and escalates through notice → warning → directive → override (claim deletion). Call this regularly to stay visible and aware.',
    inputSchema: { type: 'object', properties: {}, required: [] } },
  { name: 'fleet_claim',
    description: 'Claim a task before starting ANY work. The Queen validates no conflicts and detects file overlaps with other bees — check the response for a "conflicts" field listing files another bee is also editing. MUST claim before doing work. MUST call fleet_release() when done — NEVER write "idle", "done", or "complete" to claim files. The Queen will escalate and delete stale claims automatically.',
    inputSchema: { type: 'object', properties: { task: { type: 'string', description: 'One-line description of what you are working on' } }, required: ['task'] } },
  { name: 'fleet_release',
    description: 'Release your current claim when done. NEVER write "idle" or "done" to claim files — always use this tool. Call fleet_journal() first to log what you accomplished. The Queen will override and delete your claim if you forget.',
    inputSchema: { type: 'object', properties: {}, required: [] } },
  { name: 'fleet_journal',
    description: 'Log completed work to the shared journal. MUST call before fleet_release when you finish something meaningful. Include what you accomplished in 1-3 bullets.',
    inputSchema: { type: 'object', properties: { entry: { type: 'string', description: 'What you accomplished (1-3 bullets)' } }, required: ['entry'] } },
  { name: 'fleet_check_inbox',
    description: 'Check for pending messages from the Queen or other bees. Returns and clears your inbox. Call when fleet_ping shows pending messages.',
    inputSchema: { type: 'object', properties: {}, required: [] } },
  { name: 'fleet_lock',
    description: 'Acquire an exclusive lease on a shared resource. Other bees cannot lock it until you unlock or the lease expires.',
    inputSchema: { type: 'object', properties: {
      resource: { type: 'string', description: 'Name of the resource to lock' },
      lease_seconds: { type: 'number', description: 'How long to hold the lock (default 300s)' }
    }, required: ['resource'] } },
  { name: 'fleet_unlock',
    description: 'Release a lock on a shared resource.',
    inputSchema: { type: 'object', properties: { resource: { type: 'string', description: 'Name of the resource to unlock' } }, required: ['resource'] } },
  { name: 'fleet_announce',
    description: 'Broadcast a message to all bees in your hive. Use for discoveries, warnings, or coordination that affects everyone.',
    inputSchema: { type: 'object', properties: { message: { type: 'string', description: 'Message to broadcast' } }, required: ['message'] } },
  { name: 'fleet_request_review',
    description: 'Request the Queen to review your completed work before final output. Include a summary of what you did.',
    inputSchema: { type: 'object', properties: { summary: { type: 'string', description: 'Summary of the work to review' } }, required: ['summary'] } }
];

function handleMcp(msg, beeId, hiveId) {
  const id = msg.id;
  if (msg.method === 'initialize') {
    return { jsonrpc: '2.0', id, result: { protocolVersion: '2024-11-05', capabilities: { tools: {} }, serverInfo: { name: 'fleetode', version: '0.1.0' } } };
  }
  ...
  if (msg.method === 'tools/call') {
    ...
    const text = mcpToolCall(name, args || {}, beeId, hiveId);
    return { jsonrpc: '2.0', id, result: { content: [{ type: 'text', text }] } };
  }
  return { jsonrpc: '2.0', id, error: { code: -32601, message: 'Method not found' } };
}

function resolveHive(beeId, hiveId) {
  const cfg = globalConfig();
  const hives = cfg.hives || [];
  if (hiveId) {
    const match = hives.find(h => h.name === hiveId || path.basename(h.path) === hiveId);
    if (match) return match.path;
  }
  for (const h of hives) {
    const beePath = path.join(h.path, beeId);
    try {
      const st = fs.statSync(beePath);
      if (st.isDirectory()) return h.path;
    } catch {}
  }
  return null;
}

function mcpToolCall(name, args, beeId, hiveId) {
  const hivePath = resolveHive(beeId, hiveId);

  if (name === 'fleet_ping') {
    let onboarding = null;
    if (hivePath && queen) onboarding = queen.recordHeartbeat(beeId, hivePath);
    const pending = (hivePath && queen) ? queen.pendingForBee(beeId, hivePath) : [];
    const result = { status: 'alive', bee: beeId, hive: hivePath || 'unresolved', pending };
    if (onboarding) result.onboarding = onboarding;
    if (hivePath) {
      const claimFile = path.join(hivePath, '.fleet', 'active', `${beeId}.md`);
      try {
        const content = fs.readFileSync(claimFile, 'utf8').trim().toLowerCase();
        if (content.includes('idle') || content.includes('done') || content.includes('complete'))
          result.instruction = 'Your claim contains "idle"/"done" — call fleet_release() immediately to avoid escalation.';
      } catch {}
      // Situational awareness: what other bees are doing
      const activeDir = path.join(hivePath, '.fleet', 'active');
      const others = [];
      try {
        for (const f of fs.readdirSync(activeDir)) {
          if (!f.endsWith('.md') || f === `${beeId}.md`) continue;
          const task = fs.readFileSync(path.join(activeDir, f), 'utf8').trim();
          others.push({ bee: f.replace('.md', ''), task });
        }
      } catch {}
      if (others.length) result.activeBees = others;
    }
    return JSON.stringify(result);
  }

  if (!hivePath) return JSON.stringify({ status: 'error', message: `No hive found for bee "${beeId}". Set X-Hive-Id header.` });

  if (name === 'fleet_claim') {
    const validation = queen ? queen.validateClaim(beeId, hivePath, args.task) : { approved: true, activeBees: [] };
    const activeDir = path.join(hivePath, '.fleet', 'active');
    try { fs.mkdirSync(activeDir, { recursive: true }); } catch {}
    fs.writeFileSync(path.join(activeDir, `${beeId}.md`), `Working on: ${args.task}\n`);
    if (queen) queen.recordClaim(beeId, hivePath, args.task);
    const result = { status: 'claimed', bee: beeId, task: args.task, activeBees: validation.activeBees };
    if (validation.conflicts) result.conflicts = validation.conflicts;
    return JSON.stringify(result);
  }

  if (name === 'fleet_release') {
    try { fs.unlinkSync(path.join(hivePath, '.fleet', 'active', `${beeId}.md`)); } catch {}
    if (queen) queen.recordRelease(beeId, hivePath);
    return JSON.stringify({ status: 'released', bee: beeId });
  }

  if (name === 'fleet_journal') {
    const date = new Date().toISOString().split('T')[0];
    fs.appendFileSync(path.join(hivePath, '.fleet', 'journal.md'), `\n## ${date} — ${beeId}\n- ${args.entry}\n`);
    return JSON.stringify({ status: 'journaled', bee: beeId, date });
  }

  if (name === 'fleet_check_inbox') {
    if (!queen) return JSON.stringify({ messages: [] });
    const messages = queen.readInbox(beeId, hivePath);
    queen.clearInbox(beeId, hivePath);
    return JSON.stringify({ messages });
  }

  if (name === 'fleet_lock') {
    if (!queen) return JSON.stringify({ status: 'error', message: 'Queen not active' });
    const result = queen.acquireLease(args.resource, beeId, hivePath, args.lease_seconds || 300);
    return JSON.stringify(result);
  }

  if (name === 'fleet_unlock') {
    if (!queen) return JSON.stringify({ status: 'error', message: 'Queen not active' });
    const result = queen.releaseLease(args.resource, beeId, hivePath);
    return JSON.stringify(result);
  }

  if (name === 'fleet_announce') {
    if (queen) queen.announce(hivePath, args.message, beeId);
    return JSON.stringify({ status: 'announced', bee: beeId });
  }

  if (name === 'fleet_request_review') {
    if (queen) {
      queen.logEvent(hivePath, { type: 'review_request', bee: beeId, summary: args.summary });
      queen.enqueueInbox(beeId, hivePath, { type: 'review_queued', text: 'Review request received. The Queen will review your work.' });
      const droneId = queen.spawnDrone(hivePath, beeId,
        `Review the recent changes made by ${beeId}. Summary: ${args.summary}. ` +
        `Check for: bugs, missed edge cases, style issues, conflicts with other bees' work. ` +
        `Write your findings as a concise review to stdout.`
      );
      if (droneId) queen.enqueueInbox(beeId, hivePath, { type: 'review_started', text: `Review drone ${droneId} is examining your work.` });
    }
    return JSON.stringify({ status: 'review_requested', bee: beeId });
  }
...
const server = http.createServer(async (req, res) => {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, X-Bee-Id, X-Hive-Id');
  ...
  if (p === '/mcp' && method === 'POST') {
    const beeId = req.headers['x-bee-id'] || 'unknown';
    const hiveId = req.headers['x-hive-id'] || null;
    let body = '';
    req.on('data', chunk => body += chunk);
    req.on('end', () => {
      try {
        const msg = JSON.parse(body);
        const result = handleMcp(msg, beeId, hiveId);
  ...
  if (p === '/api/queen/status' && method === 'GET') {
    json(res, queen ? queen.getStatus() : { alive: false });
    return;
  }

  if (p.startsWith('/api/queen/hive/') && method === 'GET') {
    const hiveName = decodeURIComponent(p.replace('/api/queen/hive/', ''));
    const cfg = globalConfig();
    const hive = (cfg.hives || []).find(h => h.name === hiveName || path.basename(h.path) === hiveName);
    if (!hive || !queen) { json(res, { error: 'Not found' }, 404); return; }
    json(res, queen.hiveStatus(hive.path));
    return;
  }

  if (p === '/api/queen/decree' && method === 'POST') {
    let body = '';
    req.on('data', chunk => body += chunk);
    req.on('end', () => {
      try {
        const { hive: hiveName, message } = JSON.parse(body);
        const cfg = globalConfig();
        const hive = (cfg.hives || []).find(h => h.name === hiveName || path.basename(h.path) === hiveName);
        if (!hive || !queen) { json(res, { error: 'Hive not found' }, 404); return; }
        queen.announce(hive.path, message, 'royal_decree');
        // Also inject via tmux to all bees with panes
        try {
          for (const e of fs.readdirSync(hive.path, { withFileTypes: true })) {
            if (e.isDirectory() && e.name.startsWith('bee')) {
              queen.injectViaTmux(e.name, hive.path, `ROYAL DECREE: ${message}`);
            }
          }
        } catch {}
        json(res, { status: 'broadcast', hive: hiveName });
      } catch { json(res, { error: 'Invalid request' }, 400); }
    });
    return;
  }

  if (p === '/api/queen/notifications' && method === 'GET') {
    if (!queen) { json(res, []); return; }
    const pending = url.searchParams.get('pending') === 'true';
    const unread = url.searchParams.get('unread') === 'true';
    json(res, queen.getNotifications({ pending, unread }));
    return;
  }

  const notifReadMatch = p.match(/^\/api\/queen\/notifications\/(notif-\d+)\/read$/);
  if (notifReadMatch && method === 'POST') {
    if (!queen) { json(res, { error: 'Queen not active' }, 400); return; }
    const notif = queen.markRead(notifReadMatch[1]);
    json(res, notif ? { ok: true } : { error: 'Not found' });
    return;
  }

  const notifActionMatch = p.match(/^\/api\/queen\/notifications\/(notif-\d+)\/action$/);
  if (notifActionMatch && method === 'POST') {
    if (!queen) { json(res, { error: 'Queen not active' }, 400); return; }
    const body = await parseBody(req);
    if (!body.actionId) { json(res, { error: 'Missing actionId' }, 400); return; }
    const notif = queen.resolveNotification(notifActionMatch[1], body.actionId);
    json(res, notif ? { ok: true, resolved: notif.result } : { error: 'Not found or already resolved' });
    return;
  }

  if (p === '/api/queen/doctor-all' && method === 'POST') {
    const cfg = globalConfig();
    const results = [];
    for (const hive of (cfg.hives || [])) {
      try {
        const out = runFleet(hive.path, 'doctor');
        results.push({ hive: hive.name, path: hive.path, output: out, ok: true });
      } catch (e) {
        results.push({ hive: hive.name, path: hive.path, output: e.message, ok: false });
      }
    }
    json(res, { results });
    return;
  ...
  if (p === '/api/queen/config' && method === 'PUT') {
    let body = '';
    req.on('data', chunk => body += chunk);
    req.on('end', () => {
      try {
        const { hive: hiveName, config } = JSON.parse(body);
        const cfg = globalConfig();
        const hive = (cfg.hives || []).find(h => h.name === hiveName || path.basename(h.path) === hiveName);
        if (!hive) { json(res, { error: 'Hive not found' }, 404); return; }
...
if (require.main === module) {
  queen = new Queen(globalConfig, liveClaudeInstances);
  queen.start();
  server.listen(PORT, '127.0.0.1', () => {
    console.log(`\x1b[32mFleet dashboard running at http://localhost:${PORT}\x1b[0m`);
    console.log(`\x1b[33mQueen supervisor active\x1b[0m`);
  });
}
