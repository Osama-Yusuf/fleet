  ...
  gitSyncBee(beeId, beeDir, hivePath) {
    try {
      ...
      if (!dirty) {
        ...
        // Also try tmux injection for immediate awareness
        const beePath = path.resolve(beeDir);
        const target = this.tmuxPanes.get(beePath);
        if (target) {
          try {
            const msg = `QUEEN: You are ${behindCount} commits behind origin/${defaultBranch}. Commit or stash your changes and pull.`;
            execSync(`tmux send-keys -t '${target}' '${msg.replace(/'/g, "'\\''")}' Enter`, { timeout: 5000 });
          } catch {}
        }
      }
    } catch {}
    ...
    try {
      for (const e of fs.readdirSync(hivePath, { withFileTypes: true })) {
        if (e.isDirectory() && e.name.startsWith('bee')) {
          ...
          if (target) {
            try {
              const escaped = `${label}: ${message}`.replace(/'/g, "'\\''");
              execSync(`tmux send-keys -t '${target}' '${escaped}' Enter`, { timeout: 5000 });
            } catch {}
          }
        }
